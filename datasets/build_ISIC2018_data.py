...
def data_preprocess(imgs_dir, annotations_dir):
    ...
    process_progress = 0
    process_duration = len(annotations_names)

    for annotation_name in annotations_names:
        process_progress += 1
        if process_progress % 50 == 0:
            print("annotations data preprocess progress: {:.2f}%".format(process_progress/ process_duration))

        annotation = cv2.imread(annotation_name, -1)
        annotation = cv2.resize(annotation, (250, 250))
        annotation = annotation // 255
        os.remove(annotation_name)
        cv2.imwrite(annotation_name.replace("_segmentation", ""), annotation)

    process_progress = 0
    process_duration = len(imgs_names)

    for img_name in imgs_names:
        process_progress += 1
        if process_progress % 50 == 0:
            print("images data preprocess progress: {:.2f}%".format(process_progress/ process_duration))
...
def load_generated_dataset(training_images_dir, training_masks_dir, validation_images_dir, validation_masks_dir):
    annotations_dir = './ISIC2018/annotations'
    images_dir = './ISIC2018/images'
    ...
    x_train, x_test, y_train, y_test = train_test_split(data_frame['image'], data_frame['mask'], test_size=0.05,
...
def isDataAlreadyGenerated():
    return (os.path.exists('./ISIC2018/annotations')) or (os.path.exists('./ISIC2018/images'))


def main(unused_argv):
    tf.gfile.MakeDirs(FLAGS.output_dir)
    if generated_dataset_load and not isDataAlreadyGenerated():
