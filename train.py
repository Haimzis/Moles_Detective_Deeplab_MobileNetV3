...
flags.DEFINE_string('train_logdir', './train_log/{0}_{1}/'.
                    format(today.strftime("%d_%m_%Y"), str(datetime.now())[11:19].replace(':', '_')),
                    'Where the checkpoint and logs are stored.')

flags.DEFINE_integer('log_steps', 10,
                     'Display logging information at every log_steps.')

flags.DEFINE_integer('save_interval_secs', 2000,
                     'How often, in seconds, we save the model to disk.')

flags.DEFINE_integer('save_summaries_secs', 2000,
...
flags.DEFINE_float('base_learning_rate', .09,
...
flags.DEFINE_integer('learning_rate_decay_step', 750,
                     'Decay the base learning rate at a fixed step.')

flags.DEFINE_float('learning_power', 0.9,
                   'The power value used in the poly learning policy.')

flags.DEFINE_integer('training_number_of_steps', 110000,
...
flags.DEFINE_list('train_crop_size', '250,250',
...
flags.DEFINE_integer('output_stride', 16,  # remember to change it to 1
...
def main(unused_argv):
    ...
    if not FLAGS.tf_initial_checkpoint:
    ...
    with tf.Graph().as_default() as graph:
        ...
        with contrib_tfprof.ProfileContext(
            ...
            if FLAGS.tf_initial_checkpoint:
                ...
                slim.learning.train(
                    train_tensor,
                    logdir=FLAGS.tf_initial_checkpoint,
