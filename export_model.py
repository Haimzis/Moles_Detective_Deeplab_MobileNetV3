...
CHECKPOINT_DIR =my_params.CHECKPOINT_DIR
flags.DEFINE_string('checkpoint_path', '{checkpoint_path}'.format(checkpoint_path=find_most_updated_weights_file(CHECKPOINT_DIR)), 'Checkpoint path')

flags.DEFINE_string('export_path', my_params.EXPORT_MODEL_PATH,
                    'Path to output Tensorflow frozen graph.')

flags.DEFINE_integer('num_classes', 2, 'Number of classes.')

flags.DEFINE_multi_integer('crop_size', [my_params.INPUT_SIZE, my_params.INPUT_SIZE],
...
def _create_input_tensors():
  ...
  input_image = tf.placeholder(tf.uint8, [1, my_params.INPUT_SIZE, my_params.INPUT_SIZE, my_params.INPUT_CHANNELS], name=_INPUT_NAME)
...
def main(unused_argv):
  tf.logging.info('#### EXPORTING PB MODEL ####')
  ...
  with tf.Graph().as_default():
    ...
    tf.logging.info('#### EXPORTING TFLITE MODEL ####')
    export_model_as_tflite(FLAGS.export_path)
